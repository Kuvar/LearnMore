...
namespace LearnMore.Controllers
{
    ...
    public class AccountController : Controller
    {
        ...
        public ActionResult Login(LoginModel model, string returnUrl)
        {
            ...
            {
                User user = _usersRepository.Logon(model.UserName, model.Password);
                if (user != null)
                {
                    FormsAuthenticationTicket tkt = new FormsAuthenticationTicket(1, user.FName, DateTime.Now, DateTime.Now.AddDays(2), false, user.Role.RoleName);
        ...
        {
            ViewBag.panelTitle = "Category";
            return View();
        }

        public ActionResult CategoryList(GridSettings pGridsettings)
        {
            var categoryList = _categoryRepository.GetRecords(pGridsettings);
            var totalCategory = _categoryRepository.CountRecords(pGridsettings);

            var jsonData = new
            {
                total = totalCategory / pGridsettings.PageSize + 1,
                page = pGridsettings.PageIndex,
                records = totalCategory,
                rows = (
        ...
        public ActionResult GetCategoryList()
        {
            ...
            var jsonData =
                new {rows = _categoryRepository.All().Select(c => new {c.Id, c.Name, c.UrlSlug, c.Description}).ToArray()};
        ...
        {
            ViewBag.panelTitle = "Tags";
            return View();
        }

        public JsonResult GetTagList()
        {
            var tagList = _tagRepository.All();

            List<TagModel> objList = new List<TagModel>();
