...
namespace LearnMore
{
    public static class ActionLinkExtensions
    {
        ...
        {
            return helper.ActionLink(category.Name, "Category", "Blog", new { id = category.UrlSlug }, new { title = String.Format("See all posts in {0}", category.Name) });
        }

        public static MvcHtmlString TagLink(this HtmlHelper helper, Tag tag)
        {
            return helper.ActionLink(tag.Name, "Tag", "Blog", new { id = tag.UrlSlug }, new { title = String.Format("See all posts in {0}", tag.Name) });
        }
    }
}
